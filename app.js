...
const navGroups = [
  ...
  { label: 'DOCUMENT HUB', items: [['academic', 'งานวิชาการ', 'file'], ['orders', 'คำสั่งโรงเรียน', 'file'], ['forms', 'แบบฟอร์มทั่วไป', 'file']] },
...
const DEFAULT_SETTINGS = { teacher_name: 'ครูเฉลิมพล จันทร์แดง', school_name: 'โรงเรียนบ้านไร่', teacher_role: 'ครูผู้สอน', teacher_initial: 'ช', teacher_bio: 'สร้างการเรียนรู้ สนุก ทันสมัย และเติบโตไปพร้อมผู้เรียน', subjects: 'เทคโนโลยี|วิทยาการคำนวณ|การงานอาชีพ|ต้านทุจริตศึกษา', show_dashboard: 'true', show_profile: 'true', show_teaching: 'true', show_schedule: 'true', show_workload: 'true', show_special_tasks: 'true', show_portfolio: 'true', show_documents: 'false' };
...
function isPublicRouteVisible(route) {
  if (isAdmin()) return true;
  const keys = { dashboard: 'show_dashboard', profile: 'show_profile', teaching: 'show_teaching', schedule: 'show_schedule', workload: 'show_workload', 'special-tasks': 'show_special_tasks', portfolio: 'show_portfolio', academic: 'show_documents', orders: 'show_documents', forms: 'show_documents' };
  return setting(keys[route] || '') === 'true';
}
...
function renderNavigation() {
  $('#navigation').innerHTML = navGroups.filter(group => !group.adminOnly || isAdmin()).map(group => ({ ...group, items: group.items.filter(([route]) => isPublicRouteVisible(route)) })).filter(group => group.items.length).map(group => `
...
function setRoute(route, label = '') {
  if (!isPublicRouteVisible(route) || (!isAdmin() && route === 'settings')) route = isPublicRouteVisible('dashboard') ? 'dashboard' : 'profile';
  state.route = route;
...
function renderDashboard() {
  ...
  return `<section class="premium-hero relative mb-7 overflow-hidden rounded-[2rem] bg-gradient-to-br from-indigo-700 via-violet-600 to-fuchsia-500 p-6 text-white sm:p-9"><div class="absolute -right-10 -top-12 h-52 w-52 rounded-full bg-amber-200/40 blur-3xl"></div><div class="absolute -bottom-20 left-1/3 h-48 w-48 rounded-full bg-cyan-200/40 blur-3xl"></div><div class="relative flex flex-col gap-7 md:flex-row md:items-center md:justify-between"><div class="flex items-center gap-5"><div class="relative grid h-20 w-20 shrink-0 place-items-center rounded-[1.65rem] border border-white/60 bg-white/15 text-3xl font-bold shadow-xl ring-8 ring-white/10">ช<span class="absolute -bottom-1 -right-1 h-5 w-5 rounded-full border-4 border-violet-600 bg-emerald-400"></span></div><div><div class="inline-flex items-center gap-2 rounded-full border border-white/25 bg-white/10 px-3 py-1 text-xs font-medium text-white/90">✦ Professional Teacher Portfolio</div><p class="mt-3 text-sm font-medium text-white/80">Teacher E-Portfolio · โรงเรียนบ้านไร่</p><h2 class="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">ครูเฉลิมพล จันทร์แดง</h2><p class="mt-2 max-w-xl text-sm leading-6 text-white/85">สร้างการเรียนรู้ สนุก ทันสมัย และเติบโตไปพร้อมผู้เรียน</p></div></div><div class="flex shrink-0 gap-2"><button class="rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-violet-700 shadow-lg shadow-violet-950/15 transition hover:-translate-y-0.5 hover:bg-violet-50" data-go="portfolio">ชมผลงาน <span class="ml-1">↗</span></button>${isAdmin() ? '<button class="rounded-xl border border-white/35 bg-white/10 px-4 py-2.5 text-sm font-medium text-white backdrop-blur hover:bg-white/20" data-go="settings">ตั้งค่าหน้าเว็บ</button>' : ''}</div></div></section>
...
function statCard(label, value, unit, color, symbol) { return `<article class="stat-card overflow-hidden rounded-2xl border border-white/80 bg-gradient-to-br ${color} p-5 shadow-sm"><div class="flex items-start justify-between"><div><p class="text-sm font-medium text-slate-600">${label}</p><p class="mt-2 text-2xl font-semibold tracking-tight text-slate-800">${value}<span class="ml-1 text-sm font-normal text-slate-500">${unit}</span></p></div><span class="grid h-11 w-11 place-items-center rounded-2xl bg-white/85 text-lg shadow-sm ring-1 ring-white">${symbol}</span></div></article>`; }
...
function renderRecordEditor(sheet, record) {
  ...
  const upload = sheet === 'Portfolio' ? `<label class="block sm:col-span-2 rounded-xl border border-dashed border-sky-300 bg-sky-50 p-4"><span class="block text-sm font-medium text-sky-800">อัปโหลดรูปภาพปก (ไม่บังคับ)</span><span class="mt-1 block text-xs text-sky-700">ไฟล์จะถูกบันทึกลงโฟลเดอร์รูปภาพ Google Drive อัตโนมัติ · สูงสุด 4 MB</span><input name="upload-image" type="file" accept="image/*" class="mt-3 block w-full text-sm text-slate-600" /></label>` : sheet === 'Documents' ? `<label class="block sm:col-span-2 rounded-xl border border-dashed border-violet-300 bg-violet-50 p-4"><span class="block text-sm font-medium text-violet-800">อัปโหลดเอกสาร (ไม่บังคับ)</span><span class="mt-1 block text-xs text-violet-700">ไฟล์จะถูกบันทึกลงโฟลเดอร์เอกสาร Google Drive อัตโนมัติ · สูงสุด 4 MB</span><input name="upload-document" type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.zip" class="mt-3 block w-full text-sm text-slate-600" /></label>` : '';
  return `<form id="admin-record-form" class="mt-6 rounded-2xl border border-violet-100 bg-violet-50/60 p-5"><div class="flex items-center justify-between gap-3"><div><h3 class="font-semibold text-slate-800">${record ? 'แก้ไขรายการ' : 'เพิ่มรายการใหม่'} · ${sheet}</h3><p class="mt-1 text-xs text-slate-500">กรอกข้อมูลแล้วกดบันทึก ระบบจะเขียนลง Google Sheets โดยตรง</p></div><button type="button" data-admin-cancel class="rounded-lg px-3 py-2 text-sm text-slate-500 hover:bg-white">ยกเลิก</button></div><div class="mt-5 grid gap-4 sm:grid-cols-2">${fields.map(field => `<label class="block ${field === 'Description' ? 'sm:col-span-2' : ''}"><span class="mb-1.5 block text-sm font-medium text-slate-700">${fieldLabel(field)}</span>${field === 'Description' ? `<textarea name="${field}" rows="3" class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-100">${escapeHtml(record?.[field] || '')}</textarea>` : `<input name="${field}" value="${escapeHtml(record?.[field] || '')}" ${field === 'ID' && record ? 'readonly' : ''} class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-100 ${field === 'ID' && record ? 'text-slate-400' : ''}" />`}</label>`).join('')}${upload}</div><input type="hidden" name="sheet" value="${sheet}" /><button class="mt-5 rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-violet-700">บันทึกข้อมูล</button></form>`;
}
...
function renderLiveSettings() {
  ...
  const visibility = [['หน้าแดชบอร์ด', 'show_dashboard'], ['ประวัติส่วนตัว', 'show_profile'], ['ข้อมูลการสอน', 'show_teaching'], ['ตารางสอน', 'show_schedule'], ['ภาระงานหลัก', 'show_workload'], ['งานพิเศษ', 'show_special_tasks'], ['E-Portfolio', 'show_portfolio'], ['Document Hub', 'show_documents']];
  const sheets = Object.keys(ADMIN_SCHEMAS);
  return `<section class="max-w-6xl"><div class="rounded-3xl border border-violet-100 bg-white p-6 shadow-soft sm:p-8"><div class="flex items-start gap-4"><div class="grid h-12 w-12 place-items-center rounded-2xl bg-violet-100 text-violet-700">${icons.settings}</div><div><h2 class="font-semibold text-slate-800">ศูนย์ควบคุมเว็บไซต์</h2><p class="mt-1 text-sm leading-6 text-slate-500">แก้ไขข้อมูลเว็บไซต์และกำหนดส่วนที่แสดงต่อผู้ชมได้จากที่เดียว</p></div></div><form id="site-settings-form" class="mt-6 grid gap-4 sm:grid-cols-2">${siteSettings.map(([label, key]) => `<label class="block ${key === 'teacher_bio' || key === 'subjects' ? 'sm:col-span-2' : ''}"><span class="mb-1.5 block text-sm font-medium text-slate-700">${label}</span>${key === 'teacher_bio' ? `<textarea name="${key}" rows="2" class="w-full rounded-xl border border-slate-200 px-3 py-2 outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-100">${escapeHtml(setting(key))}</textarea>` : `<input name="${key}" value="${escapeHtml(setting(key))}" class="w-full rounded-xl border border-slate-200 px-3 py-2 outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-100" />`}</label>`).join('')}<div class="sm:col-span-2 rounded-2xl bg-sky-50 p-4"><h3 class="font-semibold text-sky-900">การแสดงผลสำหรับผู้ชม</h3><p class="mt-1 text-sm text-sky-700">ปิดสวิตช์เพื่อซ่อนเมนูและหน้าเนื้อหานั้นจากผู้เข้าชมทั่วไป</p><div class="mt-4 grid gap-3 sm:grid-cols-2">${visibility.map(([label, key]) => `<label class="flex cursor-pointer items-center justify-between rounded-xl bg-white px-4 py-3 text-sm text-slate-700 shadow-sm"><span>${label}</span><input name="${key}" type="checkbox" value="true" ${setting(key) === 'true' ? 'checked' : ''} class="h-4 w-4 accent-violet-600" /></label>`).join('')}</div></div><div class="sm:col-span-2"><button class="rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-violet-700">บันทึกการตั้งค่าเว็บไซต์</button></div></form></div><div class="mt-7 rounded-3xl border border-violet-100 bg-white p-6 shadow-soft"><div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><h2 class="font-semibold text-slate-800">จัดการข้อมูล</h2><p class="mt-1 text-sm text-slate-500">เพิ่ม แก้ไข ลบ และอัปโหลดไฟล์เข้า Google Drive ได้จากหน้า Admin</p></div><div class="flex flex-wrap gap-2">${sheets.map(sheet => `<button data-admin-sheet="${sheet}" class="rounded-full px-3 py-2 text-sm ${sheet === state.adminSheet ? 'bg-violet-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-violet-50'}">${sheet}</button>`).join('')}</div></div>${renderAdminTable(state.adminSheet)}</div></section>`;
}
...
function readFileAsBase64(file) { return new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result).split(',')[1]); reader.onerror = () => reject(new Error('อ่านไฟล์ไม่สำเร็จ')); reader.readAsDataURL(file); }); }
async function uploadAdminFile(file, type) { if (!file) return ''; if (file.size > 4 * 1024 * 1024) throw new Error('ไฟล์ใหญ่เกิน 4 MB กรุณาย่อไฟล์ก่อนอัปโหลด'); const base64 = await readFileAsBase64(file); const result = await adminRequest('uploadFile', { type, fileName: file.name, mimeType: file.type || 'application/octet-stream', base64 }); return result.url; }
...
function bindPageEvents() {
  ...
  const recordForm = $('#admin-record-form'); if (recordForm) recordForm.addEventListener('submit', async event => {
    event.preventDefault(); const formData = new FormData(recordForm); const record = Object.fromEntries(formData.entries()); const submitButton = recordForm.querySelector('button[type="submit"]');
    try { submitButton.disabled = true; submitButton.textContent = 'กำลังอัปโหลด/บันทึก…'; const imageFile = recordForm.elements['upload-image']?.files?.[0]; const documentFile = recordForm.elements['upload-document']?.files?.[0]; if (imageFile) record.Image_URL = await uploadAdminFile(imageFile, 'image'); if (documentFile) record.File_URL = await uploadAdminFile(documentFile, 'document'); delete record['upload-image']; delete record['upload-document']; await adminRequest('saveRecord', { sheet: record.sheet, record }); state.adminEditKey = null; await fetchData(true); showToast('บันทึกรายการเรียบร้อย', 'success'); }
    catch (error) { showToast(error.message); }
    finally { submitButton.disabled = false; submitButton.textContent = 'บันทึกข้อมูล'; }
  });
  const settingsForm = $('#site-settings-form'); if (settingsForm) settingsForm.addEventListener('submit', async event => {
    event.preventDefault(); const formData = new FormData(settingsForm); const visibilityKeys = ['show_dashboard', 'show_profile', 'show_teaching', 'show_schedule', 'show_workload', 'show_special_tasks', 'show_portfolio', 'show_documents']; const values = Object.fromEntries(formData.entries()); visibilityKeys.forEach(key => { values[key] = settingsForm.elements[key].checked ? 'true' : 'false'; }); const records = Object.entries(values).map(([Key, Value]) => ({ Key, Value }));
